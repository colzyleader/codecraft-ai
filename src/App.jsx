...
function MainApp({ user, onLogout, theme, toggleTheme, onPricing }) {
  ...
  useEffect(() => { if (activeChatId && messages.length > 0) { store.saveChat(user.uid, { id: activeChatId, title: messages[0]?.content?.slice(0, 50) || "New Chat", messages, updatedAt: Date.now() }); setChatList(store.getChats(user.uid)); } }, [messages, activeChatId]);
