...
import { signInWithGoogle, logOut, onAuthChange, getIdToken, subscribeCredits, serverCredits } from "./firebase";
...
const MODELS = [
  { id: "claude-sonnet-4-6-20250217", name: "Sonnet 4.6", provider: "anthropic", cost: 2, icon: "✦" },
  { id: "claude-opus-4-6", name: "Opus 4.6", provider: "anthropic", cost: 5, icon: "◆" },
  ...
  { id: "gemini-3.1-pro-preview", name: "Gemini 3.1 Pro", provider: "gemini", cost: 3, icon: "◈" },
  { id: "gemini-2.5-flash", name: "Gemini 2.5 Flash", provider: "gemini", cost: 1, icon: "◇" },
];
...
/* ═══ LOCAL STORAGE (chats + theme only — auth & credits now in Firebase) ═══ */
const store = {
  _chatKey(uid) { return "cc_chats_" + uid; },
  getChats(uid) { try { return JSON.parse(localStorage.getItem(this._chatKey(uid)) || "[]"); } catch { return []; } },
  saveChat(uid, chat) { const c = this.getChats(uid); const i = c.findIndex(x => x.id === chat.id); if (i !== -1) c[i] = chat; else c.unshift(chat); localStorage.setItem(this._chatKey(uid), JSON.stringify(c.slice(0, 50))); },
  deleteChat(uid, id) { localStorage.setItem(this._chatKey(uid), JSON.stringify(this.getChats(uid).filter(c => c.id !== id))); },
      ...
      <section className="hp-section" id="pricing"><h2 className="hp-section-title">Simple, Transparent Pricing</h2><p className="hp-section-sub">Start free, upgrade as you grow. No hidden fees.</p>
        <div className="pricing-grid">{PLANS.map((p,i) => (
          <div key={i} className={"pricing-card" + (p.popular ? " popular" : "")}>
            ...
            <button className={p.popular ? "pricing-btn pricing-btn-primary" : "pricing-btn"} onClick={() => { if (p.price === 0) onGetStarted(); else if (p.stripeUrl) window.location.href = p.stripeUrl; }}>{p.price === 0 ? "Get Started Free" : "Subscribe"}</button>
          </div>
        ))}</div>
      </section>
...
/* ═══ AUTH (Real Firebase Google Sign-In) ═══ */
function AuthPage({ onLogin, onBack }) {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      else if (e.code === "auth/popup-blocked") setErr("Popup blocked — allow popups for this site.");
      else setErr("Sign-in failed: " + (e.message || "Try again."));
    }
    setLoading(false);
  };
  ...
  return (
    <div className="a-page"><div className="a-grid"/><div className="a-glow"/>
      <div className="a-box"><button className="a-back" onClick={onBack}>{"\u2190"} Back</button>
        ...
        <h1 className="a-title">Welcome to CodeCraft</h1>
        <p className="a-sub">Sign in with Google to start building</p>
        <button className="a-google" onClick={handleGoogle} disabled={loading} style={{opacity: loading ? 0.6 : 1}}>
          {loading ? "Signing in..." : <><GoogleSVG/> Continue with Google</>}
        </button>
        {err && <p className="a-err" style={{marginTop:16}}>{err}</p>}
        <p style={{marginTop:24,fontSize:12,color:"var(--text3)",textAlign:"center"}}>By signing in you agree to our Terms & Privacy Policy.</p>
      </div>
    </div>
  );
}
...
function MainApp({ user, onLogout, theme, toggleTheme, onPricing }) {
  const [chatList, setChatList] = useState(() => store.getChats(user.uid));
  ...
  const [selectedModel, setSelectedModel] = useState("claude-sonnet-4-6-20250217");
  const [modelOpen, setModelOpen] = useState(false);
  const [attachedImages, setAttachedImages] = useState([]);
  const fileInputRef = useRef(null);
  const modelRef = useRef(null);
  const currentModel = MODELS.find(m => m.id === selectedModel) || MODELS[0];
  const planData = creditData ? (PLANS.find(p => p.id === creditData.plan) || PLANS[0]) : PLANS[0];
  ...
  useEffect(() => {
    (async () => {
      const data = await serverCredits("check");
      if (data?.ok) { setCredits(data.remaining); setCreditData(data.credits); }
    })();
  }, []);
  ...
  useEffect(() => {
    if (!user.uid) return;
    const unsub = subscribeCredits(user.uid, (data) => {
      if (data) {
        const monthly = Math.max(0, (data.monthly_limit || 0) - (data.monthly_used || 0));
        const daily = Math.max(0, (data.daily_limit || 0) - (data.daily_used || 0));
        setCredits(monthly + daily);
        setCreditData(data);
      }
    });
    return unsub;
  ...
  useEffect(() => { if (activeChatId && messages.length > 0) { const firstMsg = messages[0]; const title = firstMsg?.text || (typeof firstMsg?.content === "string" ? firstMsg.content.slice(0, 50) : "New Chat"); store.saveChat(user.uid, { id: activeChatId, title, messages: messages.map(m => ({ ...m, content: typeof m.content === "string" ? m.content : m.text || "Image", images: m.images || undefined, text: m.text || undefined, role: m.role, thinking: m.thinking, code: m.code, done: m.done, hasCode: m.hasCode })), updatedAt: Date.now() }); setChatList(store.getChats(user.uid)); } }, [messages, activeChatId]);
  ...
  const handleImageAttach = (files) => {
    for (const file of files) {
      ...
      reader.onload = (e) => {
        ...
        setAttachedImages(prev => [...prev, { base64: dataUrl.split(",")[1], mediaType: file.type, preview: dataUrl, name: file.name }]);
      };
  ...
  useEffect(() => {
    const h = (e) => { const items = e.clipboardData?.items; if (!items) return; for (const item of items) { if (item.type.startsWith("image/")) { e.preventDefault(); handleImageAttach([item.getAsFile()]); break; } } };
    document.addEventListener("paste", h); return () => document.removeEventListener("paste", h);
  }, []);

  useEffect(() => {
    const drop = (e) => { e.preventDefault(); if (e.dataTransfer?.files) handleImageAttach([...e.dataTransfer.files]); };
    const over = (e) => e.preventDefault();
    document.addEventListener("drop", drop); document.addEventListener("dragover", over);
  ...
  const send = useCallback(async text => {
    const tr = (text || input).trim();
    const hasImages = attachedImages.length > 0;
    if ((!tr && !hasImages) || loading) return;
    if (user.email !== "himc798@gmail.com" && credits < currentModel.cost) { setShowUpgrade(true); return; }
    ...
    if (hasImages) {
      ...
      if (tr) userContent.push({ type: "text", text: tr });
    } else {
      userContent = tr;
    }

    const um = { role: "user", content: userContent, text: tr, images: imgs.map(i => i.preview) };
    const nx = [...messages, um]; setMessages(nx); setInput(""); setAttachedImages([]); setLoading(true); setStreaming(null); setLoadPhase("sending");
    if (inputRef.current) inputRef.current.style.height = "auto";
    try {
      setLoadPhase("generating");
      // Deduct credits on server (skip for admin)
      if (user.email !== "himc798@gmail.com") {
        const cr = await serverCredits("use", { cost: currentModel.cost });
        if (!cr?.ok) throw new Error(cr?.error || "No credits");
        setCredits(cr.remaining);
        setCreditData(cr.credits);
      }

      const controller = new AbortController();
      const timeout = setTimeout(() => controller.abort(), 300000);
      const r = await fetch("/api/chat", { method: "POST", signal: controller.signal, headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model: selectedModel, max_tokens: 12000, system: SYS, messages: nx.map(m => ({ role: m.role === "user" ? "user" : "assistant", content: Array.isArray(m.content) ? m.content : (m.content || m.text || "") })) }) });
    ...
    setLoading(false);
  }, [input, loading, messages, activeChatId, credits, currentModel, selectedModel]);

  const newChat = () => { setActiveChatId(null); setMessages([]); setPreviewCode(null); setShowPreview(false); setStreaming(null); setLoadPhase(""); setAttachedImages([]); };
  ...
  const deleteChat = id => { store.deleteChat(user.uid, id); setChatList(store.getChats(user.uid)); if (activeChatId === id) newChat(); };
  ...
  const lo = async () => { await logOut(); onLogout(); };
  ...
  const rmsg = (msg, i) => {
    if (msg.role === "user") return <div key={i} className="m-u"><div className="m-ub">
      {msg.images && msg.images.length > 0 && <div className="m-images">{msg.images.map((src,j) => <img key={j} src={src} alt="" className="m-img-thumb"/>)}</div>}
      {msg.text || (typeof msg.content === "string" ? msg.content : "")}
    </div></div>;
    return (
      <div key={i} className="m-a"><div className="m-av"><BoltSVG/></div><div className="m-ab">
        ...
        {!msg.hasCode && !msg.thinking && <div className="m-tx">{typeof msg.content === "string" ? msg.content : msg.text || ""}</div>}
      </div></div>
    );
  };
  ...
  return (
    <div className="app">
      ...
      {showUpgrade && <div className="upgrade-modal" onClick={() => setShowUpgrade(false)}><div className="upgrade-box" onClick={e => e.stopPropagation()}>
        ...
        <button onClick={() => { setShowUpgrade(false); setShowPricing(true); }}>Upgrade Plan</button>
        <button className="upgrade-close" onClick={() => setShowUpgrade(false)}>Maybe Later</button>
      </div></div>}

      {/* Pricing Modal (in-app) */}
      {showPricing && <div className="upgrade-modal" onClick={() => setShowPricing(false)}><div className="pricing-modal-box" onClick={e => e.stopPropagation()}>
        <div className="pricing-modal-hdr">
          <h2>Upgrade Your Plan</h2>
          <button className="pricing-modal-close" onClick={() => setShowPricing(false)}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
        </div>
        <p style={{color:"var(--text2)",fontSize:13,marginBottom:20}}>Current plan: <strong>{planData.name}</strong> | {credits} credits remaining</p>
        <div className="pricing-modal-grid">
          {PLANS.map((p,i) => (
            <div key={i} className={"pricing-modal-card" + (p.popular ? " popular" : "") + (creditData?.plan === p.id ? " current" : "")}>
              {p.popular && <div className="pricing-popular-badge">Most Popular</div>}
              {creditData?.plan === p.id && <div className="pricing-current-badge">Current</div>}
              <div className="pricing-name">{p.name}</div>
              <div className="pricing-price">{p.price === 0 ? "$0" : "$" + p.price}<span>/mo</span></div>
              <div className="pricing-desc">{p.desc}</div>
              <ul className="pricing-features">{p.features.map((f,j) => <li key={j}>{f}</li>)}</ul>
              <button
                className={p.popular ? "pricing-btn pricing-btn-primary" : "pricing-btn"}
                disabled={creditData?.plan === p.id}
                style={creditData?.plan === p.id ? {opacity:0.5,cursor:"default"} : {}}
                onClick={() => { if (p.price > 0 && p.stripeUrl) window.location.href = p.stripeUrl; }}
              >{creditData?.plan === p.id ? "Current Plan" : p.price === 0 ? "Free" : "Subscribe"}</button>
            </div>
          ))}
        </div>
      </div></div>}
      ...
      {sidebarOpen && <div className="sidebar">
        ...
        <div className="sb-footer">
          <div className="sb-user" onClick={lo}>
            {user.photoURL ? (
              <img src={user.photoURL} alt="" className="sb-user-av" style={{width:30,height:30,borderRadius:"50%",objectFit:"cover"}} referrerPolicy="no-referrer" />
            ) : (
      ...
      <div className="left" style={{ flex: showPreview ? "0 0 440px" : "1 1 auto", maxWidth: showPreview ? 440 : "none" }}>
        <div className="hdr">
          ...
          <div className="hdr-r">
            ...
            <button className="hdr-btn hdr-upgrade" onClick={() => setShowPricing(true)}>Upgrade</button>
        ...
        <div className="ia">
          ...
          {attachedImages.length > 0 && <div className="img-previews">
            {attachedImages.map((img,i) => <div key={i} className="img-prev-item">
              <img src={img.preview} alt=""/>
              <button className="img-prev-x" onClick={() => setAttachedImages(prev => prev.filter((_,j)=>j!==i))}>×</button>
            </div>)}
          </div>}
          ...
          <div className="ib">
            <button className="img-attach-btn" onClick={() => fileInputRef.current?.click()} title="Attach image" disabled={loading}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></button>
            <div className="model-pick" ref={modelRef}>
              ...
              {modelOpen && <div className="model-drop">
                ...
                <div className="model-drop-label" style={{marginTop:4}}>Google</div>
            ...
            <button className="is" onClick={() => send()} disabled={loading || (!input.trim() && attachedImages.length === 0)} style={{ opacity: (input.trim() || attachedImages.length > 0) && !loading ? 1 : 0.25 }}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg></button>
          </div>
...
export default function App() {
  ...
  useEffect(() => {
    const unsub = onAuthChange((u) => {
      ...
      else { setUser(null); if (page !== "auth") setPage("home"); }
    });
    return unsub;
  }, []);

  // Stripe success — update plan on server
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    if (params.get("plan") === "success" && params.get("session_id") && user) {
      (async () => {
  ...
  if (page === "loading") {
    return <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", background: "var(--bg)" }}><div className="dots"><span/><span/><span/></div></div>;
  }
