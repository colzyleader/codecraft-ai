...
5. IMAGES: MANDATORY — use real relevant images from https://source.unsplash.com/600x400/?KEYWORD (e.g. https://source.unsplash.com/600x400/?burger for burgers, https://source.unsplash.com/600x400/?fried-chicken for chicken, https://source.unsplash.com/600x400/?french-fries for fries, https://source.unsplash.com/600x400/?pizza for pizza, https://source.unsplash.com/600x400/?restaurant-interior for about sections). Use DIFFERENT keywords per item so images match the content. For non-food sites use relevant keywords like ?office, ?laptop, ?team, ?dashboard, ?fitness etc. Every card/hero/gallery MUST have an <img> tag with a real URL
...
function MainApp({ user, onLogout }) {
  ...
  const send = useCallback(async text => {
    ...
    } catch (err) { setMessages(prev => [...prev, { role: "assistant", content: err.name === "AbortError" ? "Request timed out \u2014 try a shorter prompt." : "Error: " + (err.message || "Connection failed. Try again.") }]); setStreaming(null); setLoadPhase(""); }
    setLoading(false);
