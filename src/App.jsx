...
- Use https://source.unsplash.com/600x400/?KEYWORD for images (use relevant keywords like ?burger, ?chicken, ?fries, ?pizza, ?restaurant, ?office, ?laptop, ?fitness etc)
- Every card and hero MUST have an <img> tag
  ...
  const send = useCallback(async text => {
    ...
    try {
      ...
      const r = await fetch("/api/chat", { method: "POST", signal: controller.signal, headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 8000, system: SYS, messages: nx.map(m => ({ role: m.role === "user" ? "user" : "assistant", content: m.content })) }) });
