...
- For images use https://loremflickr.com/600/400/KEYWORD with relevant keywords. Every card/hero MUST have <img> with onerror="this.style.background='#333'"
...
const PLANS = [
  ...
  { id: "starter", name: "Starter", price: 25, credits: 100, daily: 0, desc: "For individuals and side projects", features: ["100 credits/month", "Credit rollover", "Priority generation", "All templates", "Email support"], priceId: "STARTER_PRICE_ID" },
  { id: "pro", name: "Pro", price: 50, credits: 200, daily: 5, desc: "For power users and production apps", features: ["200 credits/month", "5 bonus credits/day", "Fastest generation", "All templates", "Priority support"], popular: true, priceId: "PRO_PRICE_ID" },
  { id: "max", name: "Max", price: 200, credits: 800, daily: 10, desc: "For heavy users and agencies", features: ["800 credits/month", "10 bonus credits/day", "Fastest generation", "White-label export", "Dedicated support"], priceId: "MAX_PRICE_ID" },
];
      ...
      <section className="hp-section" id="pricing"><h2 className="hp-section-title">Simple, Transparent Pricing</h2><p className="hp-section-sub">Start free, upgrade as you grow. No hidden fees.</p>
        <div className="pricing-grid">{PLANS.map((p,i) => (
          <div key={i} className={"pricing-card" + (p.popular ? " popular" : "")}>
            ...
            <button className={p.popular ? "pricing-btn pricing-btn-primary" : "pricing-btn"} onClick={p.price === 0 ? onGetStarted : onPricing}>{p.price === 0 ? "Get Started Free" : "Subscribe"}</button>
          </div>
        ))}</div>
      </section>
  ...
  const send = useCallback(async text => {
    ...
    try {
      ...
      const r = await fetch("/api/chat", { method: "POST", signal: controller.signal, headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 12000, system: SYS, messages: nx.map(m => ({ role: m.role === "user" ? "user" : "assistant", content: m.content })) }) });
    ...
    <div className="app">
      ...
      <div className="left" style={{ flex: showPreview ? "0 0 440px" : "1 1 auto", maxWidth: showPreview ? 440 : "none" }}>
        ...
        <div className="ia">
          ...
          <p className="ifooter">Powered by Claude Opus 4.6 | {credits} credits remaining</p>
        </div>
      </div>
